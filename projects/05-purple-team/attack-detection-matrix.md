...
| Impact | T1490 Inhibit System Recovery | Precursor; NO emulado todavia | Regla **100180** (nivel 13) AUTORADA, despliegue pendiente | n13 | **MEJORA DISENADA** |
...
- **Tecnicas emuladas (disparo RED real)**: 6 de 8 — T1558.003, T1059.001, T1027, T1562.001, T1105, T1486.
  - No emuladas: T1558.004 (deteccion armada, falta toolkit) y T1490 (precursor no ejecutado).
- **Tecnicas emuladas y detectadas**: 5 de 6 — todas menos T1486.
- **% de cobertura de lo emulado**: **5/6 = 83,3 %** de las tecnicas realmente disparadas generaron al menos una deteccion.
...
- **Gaps abiertos**: 1 — T1486 (cifrado/renombrado masivo no visible sin FIM de frecuencia).
- **Deteccion lista pero sin emular**: 1 — T1558.004 (regla 100140 armada).
- **Mejora disenada y no desplegada**: 1 — T1490 (regla 100180 autorada, en `wazuh/rules/local_rules.xml`, despliegue+validacion pendiente de autorizacion explicita del usuario por modificar la infraestructura del SIEM).
...
- **Ciclo #2 — T1486 / ransomware (DISENADO)**: el ejercicio IR (P4) revelo que la etapa de IMPACTO no disparo nada y que el cifrado masivo directo es dificil de ver en el SIEM sin FIM de frecuencia. IMPROVE: regla **100180** (nivel 13, T1490) que caza por `commandLine` el borrado de Shadow Copies/backups (`vssadmin`/`wmic shadowcopy`/`wbadmin`/`bcdedit`) — el **precursor detectable** que casi todo ransomware ejecuta justo antes de cifrar. Pendiente de despliegue+validacion (mismo estado que tuvo el Active Response antes de autorizarse).
